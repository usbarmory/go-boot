...
// ErrTruncated is returned when a ReceiveData transfer exceeds the supplied
// buffer.
...
// newStorageSecurity decodes the member slots, failing closed on a NULL
// instance.
func newStorageSecurity(base uint64) (s *StorageSecurity, err error) {
	var fn struct {
		ReceiveData uint64
		SendData    uint64
	}
...
// GetStorageSecurity returns the first EFI_STORAGE_SECURITY_COMMAND_PROTOCOL
// instance.
...
// GetStorageSecurityByHandle resolves an EFI_STORAGE_SECURITY_COMMAND_PROTOCOL
// handle.
...
// SendData calls EFI_STORAGE_SECURITY_COMMAND_PROTOCOL.SendData(). spSpecific
// carries the TCG ComID; timeout is in 100 ns units (0 = none).
...
// ReceiveData calls EFI_STORAGE_SECURITY_COMMAND_PROTOCOL.ReceiveData(),
// returning the valid prefix (PayloadTransferSize) of the transfer.
