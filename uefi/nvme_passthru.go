...
// NVMePassThru represents an EFI_NVM_EXPRESS_PASS_THRU_PROTOCOL instance.
...
func newNVMePassThru(base uint64) (n *NVMePassThru, err error) {
	var fn struct {
		Mode     uint64
		PassThru uint64
	}
...
// GetNVMePassThruByHandle resolves an EFI_NVM_EXPRESS_PASS_THRU_PROTOCOL
// handle.
...
// submit issues one admin-queue command packet.
func (p *NVMePassThru) submit(cmd, data, completion []byte) error {
	...
	// EFI_NVM_EXPRESS_COMPLETION.DW3 holds the completion-queue Status
	// Field in bits 31:17 (SC and SCT); a nonzero value means the
...
func (p *NVMePassThru) SecurityReceive(securityProtocol uint8, comID uint16, size int) (buf []byte, err error) {
...
// SerialNumber returns the controller's 20-byte Serial Number, as-is
// (space-padded ASCII, not trimmed) so a caller can reproduce the drive's
// exact bytes.
